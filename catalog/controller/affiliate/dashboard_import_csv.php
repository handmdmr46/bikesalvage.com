...
class ControllerAffiliateDashboardImportCsv extends Controller {
	...
	public function import() {
		...
		// Language
		$this->data['heading_title_import_csv'] = $this->language->get('heading_title_import_csv');
	   ...
	   $this->data['international_shipping'] = $this->model_affiliate_dashboard_csv->getInternationalShippingMethods();
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				  ...
				  if ($this->validImport($content)) {
					  $this->data['loading'] = '<span class="wait">&nbsp;<img src="catalog/view/theme/childtheme/affiliate_image/ajax_load.gif" alt="" /></span>';
					  ...
					  foreach($this->data['csv'] as $key => $row) {
						 ...
						 $this->model_affiliate_dashboard_csv->addCsvImportProduct($this->data['csv'][$key]);
		...
		$this->data['csv_view'] = $this->model_affiliate_dashboard_csv->getCsvImportProductInfo($start, $limit);

		if($this->data['csv_view']) {
		  $import_count = $this->model_affiliate_dashboard_csv->getTotalCsvImportProducts();
		} else {
			$import_count = '';
		}
