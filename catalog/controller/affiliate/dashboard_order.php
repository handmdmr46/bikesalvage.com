<?php
class ControllerAffiliateDashboardOrder extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->language->load('affiliate/dashboard_order');

		$this->load->model('affiliate/dashboard_order');

    	$this->data['heading_title_sale'] = $this->language->get('heading_title_sale');

		$this->document->setTitle($this->language->get('heading_title_sale'));
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	$this->model_affiliate_dashboard_order->addOrder($this->request->post);
			...
			$this->redirect($this->url->link('affiliate/dashboard_order', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
  	...
  	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      	  	$this->model_affiliate_dashboard_order->editOrder($order_id, $this->request->post);
  	...
  	public function delete() {
    	...
    	if (isset($this->request->post['selected'])) {
			foreach ($this->request->post['selected'] as $order_id) {
				$this->model_affiliate_dashboard_order->deleteOrder($order_id);
			}
  	...
  	protected function getList() {
		...
		// breadcrumbs
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('affiliate/dashboard_order', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);

		$this->data['invoice'] = $this->url->link('affiliate/dashboard_order/invoice', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['insert'] = $this->url->link('affiliate/dashboard_order/insert', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delete'] = $this->url->link('affiliate/dashboard_order/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$order_total = $this->model_affiliate_dashboard_order->getTotalOrdersByAffiliateId($data, $this->affiliate->getId());

		$results = $this->model_affiliate_dashboard_order->getOrdersByAffiliateId($data, $this->affiliate->getId());
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('affiliate/dashboard_order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL')
			);

			// may add edit orders later but removed for now
			/*if (strtotime($result['date_added']) > strtotime('-' . (int)$this->config->get('config_order_edit') . ' day')) {
				$action[] = array(
					'text' => $this->language->get('text_edit'),
					'href' => $this->url->link('affiliate/dashboard_order/update', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL')
				);
			}*/
		...
		// $url filter values for filter search
		$url = '';
		...
		$this->data['sort_order'] = $this->url->link('affiliate/dashboard_order', 'token=' . $this->session->data['token'] . '&sort=o.order_id' . $url, 'SSL');
		$this->data['sort_customer'] = $this->url->link('affiliate/dashboard_order', 'token=' . $this->session->data['token'] . '&sort=customer' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('affiliate/dashboard_order', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		$this->data['sort_total'] = $this->url->link('affiliate/dashboard_order', 'token=' . $this->session->data['token'] . '&sort=o.total' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('affiliate/dashboard_order', 'token=' . $this->session->data['token'] . '&sort=o.date_added' . $url, 'SSL');
		$this->data['sort_date_modified'] = $this->url->link('affiliate/dashboard_order', 'token=' . $this->session->data['token'] . '&sort=o.date_modified' . $url, 'SSL');

		// $url filter values for pagination
		$url = '';
		...
		$pagination->url = $this->url->link('affiliate/dashboard_order', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
    	...
    	$this->data['order_statuses'] = $this->model_affiliate_dashboard_order->getOrderStatuses();

		$this->data['sort'] = $sort;
		$this->data['order'] = $order;

		$this->template = $this->config->get('config_template') . '/template/affiliate/dashboard_order_list.tpl';
  	...
  	protected function getForm() {
		...
		$this->data['heading_title_order'] = $this->language->get('heading_title_order');
		...
		if (!isset($this->request->get['order_id'])) {
			$this->data['action'] = $this->url->link('affiliate/dashboard_order/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('affiliate/dashboard_order/update', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('affiliate/dashboard_order', 'token=' . $this->session->data['token'] . $url, 'SSL');

    	if (isset($this->request->get['order_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$order_info = $this->model_affiliate_dashboard_order->getOrder($this->request->get['order_id']);
    	}
		...
		$this->data['customer_groups'] = $this->model_affiliate_dashboard_order->getCustomerGroups();
		...
		if (isset($this->request->post['customer_id'])) {
			$this->data['addresses'] = $this->model_affiliate_dashboard_order->getAddresses($this->request->post['customer_id']);
		} elseif (!empty($order_info)) {
			$this->data['addresses'] = $this->model_affiliate_dashboard_order->getAddresses($order_info['customer_id']);
		} else {
		...
		if (isset($this->request->post['order_product'])) {
			...
			$order_products = $this->model_affiliate_dashboard_order->getOrderProductsByAffiliateId($this->request->get['order_id'], $this->affiliate->getId());
		} else {
		...
		foreach ($order_products as $order_product) {
			if (isset($order_product['order_option'])) {
				...
				$order_option = $this->model_affiliate_dashboard_order->getOrderOptions($this->request->get['order_id'], $order_product['order_product_id']);
			} else {
				$order_option = array();
			}
			...
			if (isset($order_product['order_download'])) {
				...
				$order_download = $this->model_affiliate_dashboard_order->getOrderDownloads($this->request->get['order_id'], $order_product['order_product_id']);
			} else {
		...
		if (isset($this->request->post['order_voucher'])) {
			...
			$this->data['order_vouchers'] = $this->model_affiliate_dashboard_order->getOrderVouchers($this->request->get['order_id']);
		} else {
			$this->data['order_vouchers'] = array();
		}

		$this->data['voucher_themes'] = $this->model_affiliate_dashboard_order->getVoucherThemes();

		if (isset($this->request->post['order_total'])) {
      		$this->data['order_totals'] = $this->request->post['order_total'];
    	} elseif (isset($this->request->get['order_id'])) {
			// $this->data['order_totals'] = $this->model_affiliate_dashboard_order->getOrderTotals($this->request->get['order_id']);
			$this->data['order_totals'] = $this->model_affiliate_dashboard_order->getAffiliateOrderTotals($this->request->get['order_id'], $this->affiliate->getId());			
		} else {
      		$this->data['order_totals'] = array();
    	}

		$this->template = $this->config->get('config_template') . '/template/affiliate/dashboard_order_form.tpl';
		$this->children = array(
			'affiliate/common/header',
			'affiliate/common/footer'
		);
	...
	public function autocompleteProduct() {
  	...
  	public function invoice() {
		...
		foreach ($orders as $order_id) {
			$order_info = $this->model_affiliate_dashboard_order->getOrder($order_id);

			if ($order_info) {
				$store_info = $this->model_affiliate_dashboard_setting->getSetting('config', $order_info['store_id']);
				...
				$products = $this->model_affiliate_dashboard_order->getOrderProductsByAffiliateId($order_id, $this->affiliate->getId());

				foreach ($products as $product) {
					$option_data = array();

					$options = $this->model_affiliate_dashboard_order->getOrderOptions($order_id, $product['order_product_id']);
				...
				$vouchers = $this->model_affiliate_dashboard_order->getOrderVouchers($order_id);
				...
				$total_data = $this->model_affiliate_dashboard_order->getAffiliateOrderTotals($order_id, $this->affiliate->getId());
		...
		$this->template = $this->config->get('config_template') . '/template/affiliate/dashboard_order_invoice.tpl';
	...
	public function info() {
		...
		if ($order_info) {
			...
			$this->data['invoice'] = $this->url->link('affiliate/dashboard_order/invoice', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$this->data['cancel'] = $this->url->link('affiliate/dashboard_order', 'token=' . $this->session->data['token'] . $url, 'SSL');
			...
			$this->data['credit_total'] = $this->model_affiliate_dashboard_order->getTotalTransactionsByOrderId($this->request->get['order_id']);

			$this->data['reward'] = $order_info['reward'];

			$this->data['reward_total'] = $this->model_affiliate_dashboard_order->getTotalCustomerRewardsByOrderId($this->request->get['order_id']);

			$order_status_info = $this->model_affiliate_dashboard_order->getOrderStatus($order_info['order_status_id']);
			...
			$products = $this->model_affiliate_dashboard_order->getOrderProductsByAffiliateId($this->request->get['order_id'], $affiliate_id);

			foreach ($products as $product) {
				$option_data = array();

				// Options
				$options = $this->model_affiliate_dashboard_order->getOrderOptions($this->request->get['order_id'], $product['order_product_id']);
			...
			$vouchers = $this->model_affiliate_dashboard_order->getOrderVouchers($this->request->get['order_id']);
			...
			// Order Totals
			$this->data['totals'] = $this->model_affiliate_dashboard_order->getAffiliateOrderTotals($this->request->get['order_id'], $affiliate_id);
			// $this->data['totals'] = $this->model_affiliate_dashboard_order->getOrderTotals($this->request->get['order_id']);

			// Downloads
			$this->data['downloads'] = array();

			foreach ($products as $product) {
				$results = $this->model_affiliate_dashboard_order->getOrderDownloads($this->request->get['order_id'], $product['order_product_id']);
			...
			$this->template = $this->config->get('config_template') . '/template/affiliate/dashboard_order_info.tpl';
			$this->children = array(
				'affiliate/common/header',
				'affiliate/common/footer'
			);
			...
			$this->data['breadcrumbs'][] = array(
				'text'      => $this->language->get('heading_title'),
				'href'      => $this->url->link('affiliate/dashboard_not_found', 'token=' . $this->session->data['token'], 'SSL'),
				'separator' => ' :: '
			);

			$this->template = $this->config->get('config_template') . '/template/affiliate/dashboard_not_found.tpl';
			$this->children = array(
				'affiliate/common/header',
				'affiliate/common/footer'
			);
	...
	public function createInvoiceNo() {
		...
		$json = array();
     	...
     	if (!$this->affiliate->isLogged()) {
			...
			$invoice_no = $this->model_affiliate_dashboard_order->createInvoiceNo($this->request->get['order_id']);
	...
	public function history() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!$this->data['error']) {
				$this->model_affiliate_dashboard_order->addOrderHistory($this->request->get['order_id'], $this->request->post);
		...
		$results = $this->model_affiliate_dashboard_order->getOrderHistories($this->request->get['order_id'], ($page - 1) * 10, 10);
		...
		$history_total = $this->model_affiliate_dashboard_order->getTotalOrderHistories($this->request->get['order_id']);
		...
		$pagination->limit = 10;
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('affiliate/dashboard_order/history', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . '&page={page}', 'SSL');

		$this->data['pagination'] = $pagination->render();

		$this->template = $this->config->get('config_template') . '/template/affiliate/dashboard_order_history.tpl';
	...
	public function country() {
    	...
    	$country_info = $this->model_affiliate_dashboard_localisation_zone->getCountry($this->request->get['country_id']);
	...
	public function address() {
		...
		if (!empty($this->request->get['address_id'])) {
			...
			$json = $this->model_affiliate_dashboard_order->getAddress($this->request->get['address_id']);
		}
  	...
  	protected function validateForm() {
		...
		$country_info = $this->model_affiliate_dashboard_localisation_zone->getCountry($this->request->post['payment_country_id']);
		...
		if (isset($this->request->post['order_product'])) {
			...
			foreach ($this->request->post['order_product'] as $order_product) {
				$product_info = $this->model_affiliate_dashboard->getProduct($order_product['product_id']);
		...
		$this->load->model('affiliate/dashboard_localisation_zone');
			...
			$country_info = $this->model_affiliate_dashboard_localisation_zone->getCountry($this->request->post['shipping_country_id']);
