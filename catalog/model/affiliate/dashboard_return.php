<?php
class ModelAffiliateDashboardReturn extends Model {
	public function addReturn($data, $affiliate_id) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "return` 
	...
	public function getReturnsByAffiliateId($data = array(), $affiliate_id) {
		$sql = "SELECT *, 
                ...
                WHERE r.affiliate_id = '" . (int)$affiliate_id . "'";
	...
	public function getTotalReturnsByAffiliateId($data = array(), $affiliate_id) {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "return r WHERE r.affiliate_id = '" . (int)$affiliate_id . "'";
	...
	public function addReturnHistory($return_id, $data) {
		...
		if ($data['notify']) {
			...
			if ($return_query->num_rows) {
				$this->language->load('affiliate/dashboard_mail');
	...
	public function getReturnHistories($return_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT    rh.date_added, 
			                                 rs.name AS status, 
			                                 rh.comment, 
			                                 rh.notify 
