<?php
class ModelAffiliateDashboardOrder extends Model {	
	public function addOrder($data) {
		$this->load->model('affiliate/dashboard_setting');
		
		$setting_info = $this->model_affiliate_dashboard_setting->getSetting('setting', $data['store_id']);
		...
		$country_info = $this->model_affiliate_dashboard_country->getCountry($data['shipping_country_id']);
		...
		$zone_info = $this->model_affiliate_dashboard_localisation_zone->getZone($data['shipping_zone_id']);
		...
		$country_info = $this->model_affiliate_dashboard_country->getCountry($data['payment_country_id']);
		...
		$zone_info = $this->model_affiliate_dashboard_localisation_zone->getZone($data['payment_zone_id']);
		...
		$currency_info = $this->model_affiliate_dashboard_localisation_currency->getCurrencyByCode($this->config->get('config_currency'));
	...
	public function getProductAffiliateId($product_id) {
		$query = $this->db->query("SELECT `affiliate_id` FROM " . DB_PREFIX ."product WHERE `product_id` = '" . (int)$product_id . "'");
		return $query->row;
	}
	
	// BROKEN - NEEDS FIXED - orders gone after edit
	public function editOrder($order_id, $data) {
		$this->load->model('affiliate/dashboard_localisation_zone');
		
		$country_info = $this->model_affiliate_dashboard_localisation_zone->getCountry($data['shipping_country_id']);
		...
		$country_info = $this->model_affiliate_dashboard_localisation_zone->getCountry($data['payment_country_id']);
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			$language_info = $this->model_affiliate_dashboard_localisation->getLanguage($order_query->row['language_id']);
	...
	public function getOrdersByAffiliateId($data = array(), $affiliate_id) {
	...
	public function getTotalOrdersByAffiliateId($affiliate_id) {
	...
	public function getOrderProductsByAffiliateId($order_id, $affiliate_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "' AND affiliate_id = '" . (int)$affiliate_id . "'");
	...
	public function getAffiliateOrderTotals($order_id, $affiliate_id) {
	...
	public function getTotalSalesByAffiliateId($affiliate_id) {
      	$query = $this->db->query("SELECT    SUM(total) AS total 
      							   FROM      " . DB_PREFIX . "order o      							  
      							   WHERE     o.order_status_id > '0'
      							   AND       o.order_id IN (SELECT op.order_id 
      							   	                        FROM   " . DB_PREFIX . "order_product op
      							   	                        WHERE  op.affiliate_id = '" . (int)$affiliate_id . "')");
	...
	public function getTotalSalesByYearAndAffiliateId($year, $affiliate_id) {
      	$query = $this->db->query("SELECT SUM(total) AS total 
      		                       FROM   " . DB_PREFIX . "order o
      		                       WHERE  o.order_status_id > '0' 
      		                       AND    YEAR(o.date_added) = '" . (int)$year . "'
      		                       AND    o.order_id IN (SELECT op.order_id
      		                       	                     FROM   " . DB_PREFIX . "order_product op
      		                       	                     WHERE  op.affiliate_id = '" . (int)$affiliate_id . "')");
	...
	public function addOrderHistory($order_id, $data) {
		...
		$order_info = $this->getOrder($order_id);
      	...
      	if ($data['notify']) {
			...
			$language->load('affiliate/dashboard_mail');
