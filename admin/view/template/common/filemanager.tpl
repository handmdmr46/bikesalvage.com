...
<div id="container">
  <div id="menu">
  	...
  	<input type="text" id="search" style="margin-top: 5px; width:120px; float:left;"/> <a id="btnSearch" class="button" style="background-image: url('view/image/filemanager/search.png');"><?php echo $button_search; ?></a>
</div>
...
<script type="text/javascript"><!--
	$(document).ready(function() {
	                    ...
	                    $.ajax({
	                        ...
	                        success: function(response_json) {
	                            if ( response_json) {
	                                ...
	                                if ( undefined != response_json.success ) alert(response_json.success);
	                            }
	                        }
	                    });
	            });
		...
		new AjaxUpload( $('#upload, #upload_2'), {
			...
			onComplete: function(file, json) {
	            if (json.success) {
	                var tree = $.tree.focused();
	                var tree2 = $.tree.focused();

	                if ( $('#addtoimagetab').is(':checked') ) {
	                    tree.select_branch(tree.selected);
	                }

	                tree2.select_branch(tree.selected);
...
<script type="text/javascript"><!--
    $(document).ready(function() {
        $("#btnSearch").click(function () {
            var str = $("#search").val();
            if ( $("#column-right a:contains('" + str + "')").length > 0 ) {
                $('#column-right').animate(
                    ...
                    2000,
                    function() {
                        $("#column-right a:contains('" + str + "')").attr("class","selected");
                    }
                );
            }
        });
